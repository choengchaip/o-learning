import 'dart:ui';

final Color backgroundColor = Color(0xFFFFFFFF);
final Color primaryColor = Color(0xFFFF3D00);
final Color primaryColorDark = Color(0xFFAF2A00);
final Color primaryColorLight = Color(0xFFFFAD7D);
final Color dark = Color(0xFF1A212E);
final Color gray = Color(0xFF808080);
final Color grayLight = Color(0xFFC2C2C2);
final Color grayLighter = Color(0xFFE4E4E4);
final Color errorColor = Color(0xFFAF2A00);
final Color dividerColor = Color(0xFFFF3D00);